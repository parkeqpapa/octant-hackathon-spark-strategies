...
import {SparkSavingsStrategy} from "../../../src/strategies/yieldDonating/SparkSavings/SparkSavingsStrategy.sol";
import {ISparkVault} from "../../../src/interfaces/SparkSavings/SparkVault.sol";
...
contract YieldDonatingShutdownTest is Setup {
    ...
    function testLossDoesNotTriggerTooMuchLossError() public {
        ...
        // Simulate a 10% loss by transferring spark vault shares to a dead address
        SparkSavingsStrategy sparkSavingsStrategy = SparkSavingsStrategy(address(strategy));
        ISparkVault sparkVault = sparkSavingsStrategy.sparkVault();
        uint256 vaultBalanceBeforeLoss = sparkVault.balanceOf(address(strategy));
        uint256 vaultLoss = vaultBalanceBeforeLoss / 10;
        vm.prank(address(strategy));
        sparkVault.transfer(address(0xdead), vaultLoss);
        vm.stopPrank();
